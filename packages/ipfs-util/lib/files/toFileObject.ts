...
export function toFileObject (input) {
	const obj = {
		path: input.path || '',
		mode: input.mode,
		mtime: input.mtime,
...
export function blobToAsyncGenerator (blob) {
...
export async function * browserStreamToIt (stream) {
...
export async function * readBlob (blob, options?): AsyncGenerator<Buffer, void, unknown> {
	options = options || {}
